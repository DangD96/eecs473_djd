...
int main(int argc, char** argv) {
    ros::init(argc, argv, "irb120_move_gear_djd122"); // name this node 
    ...
    int nsteps = 100; //will need to specify how many interpolation points in Cartesian path; changed from 5 to 100
    ...
    //declare my goal position coordinates
	double x_aim = 0.2;
	double y_aim = 0.2;
	...
	double x_offset = 0;
	double y_offset = 0;
	double offset = 0.065; // a little larger than half of the radius of the gear part
...
//done with inquiry.  If here, then part pose is in g_perceived_object_pose.  Use it to compute robot motion 
	...
	while(ros::ok()) {
		...
		if(g_perceived_object_pose.pose.position.x > x_aim)
		{
		    x_offset = offset;
		    y_offset = 0;
		}
		else if(g_perceived_object_pose.pose.position.x < x_aim)
		{
		    x_offset = -offset;
		    y_offset = 0; 
		}
		else if(g_perceived_object_pose.pose.position.y > y_aim){
		    x_offset = 0;
		    y_offset = offset; 
		}
		else if(g_perceived_object_pose.pose.position.y < y_aim){
...
//position flange based on perceived object positions and goal positions - get to opposite side
		flange_origin << g_perceived_object_pose.pose.position.x + x_offset, 			g_perceived_object_pose.pose.position.y+y_offset, 0.02; 	   //Set the desired flange position 
...
//drop flange down while keeping x and y coordinates
		flange_origin << g_perceived_object_pose.pose.position.x + x_offset, g_perceived_object_pose.pose.position.y+y_offset, 0; 	    
...
//push object toward target by moving flange
		if(g_perceived_object_pose.pose.position.x > x_aim)
		{
			flange_origin << x_aim + x_offset, g_perceived_object_pose.pose.position.y, 0;
		}
		else if(g_perceived_object_pose.pose.position.x < x_aim)
		{
			flange_origin << x_aim + x_offset, g_perceived_object_pose.pose.position.y, 0;
			...
			flange_origin << g_perceived_object_pose.pose.position.x, y_aim + y_offset, 0; 
		}
		else if(g_perceived_object_pose.pose.position.y < y_aim){
			flange_origin << g_perceived_object_pose.pose.position.x, y_aim + y_offset, 0; 
		}
