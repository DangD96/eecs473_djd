...
ActionServerClass::ActionServerClass(ros::NodeHandle* nodehandle):nh_(*nodehandle)
{ // constructor
...
void ActionServerClass::initializeSubscribers()
{
    ROS_INFO("Initializing Subscribers");
    minimal_subscriber_ = nh_.subscribe("example_class_input_topic", 1, &ActionServerClass::subscriberCallback,this);  
    // add more subscribers here, as needed
}

//member helper function to set up services:
// similar syntax to subscriber, required for setting up services outside of "main()"
void ActionServerClass::initializeServices()
{
    ROS_INFO("Initializing Services");
    minimal_service_ = nh_.advertiseService("example_minimal_service",
                                                   &ActionServerClass::serviceCallback,
                                                   this);  
    // add more services here, as needed
}

//member helper function to set up publishers;
void ActionServerClass::initializePublishers()
{
...
void ActionServerClass::subscriberCallback(const std_msgs::Float32& message_holder) {
...
bool ActionServerClass::serviceCallback(std_srvs::TriggerRequest& request, std_srvs::TriggerResponse& response) {
...
{
    // ROS set-ups:
    ros::init(argc, argv, "action_server_class"); //node name

    ros::NodeHandle nh; // create a node handle; need to pass this to the class constructor

    ROS_INFO("main: instantiating an object of type ExampleRosClass");
    ActionServerClass actionServerClass(&nh);  //instantiate an ExampleRosClass object and pass in pointer to nodehandle for constructor to use
